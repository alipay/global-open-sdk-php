...
$request = new \Request\pay\AlipayPaymentSessionRequest();
...
Optional configuration: `connectTimeout` (15 seconds), `timeout` (30 seconds), both positive integers. cURL connections are released after each request. The application can configure its CA bundle with `curl.cainfo`.
